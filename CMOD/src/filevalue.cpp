...
void FileValue::Evaluate() {
  ...
  } else if (ftnString == "DURATION_UNITS") {
    static_ftn_DURATION_UNITS();
...
void FileValue::ftn_MakeSieve() {
  ...
  vector<int> eArgInts;
  vector<int> wArgInts;
  list<FileValue>::iterator iter = eArgs->begin();
...
void FileValue::static_ftn_DURATION_UNITS() {
  if (evptr == NULL) {
    cerr << "FileValue Error: DURATION_UNITS used outside of Event context" 
         << endl;
    exit(1);
  }
  return_type = FVAL_NUMBER;
  n = evptr->getDurationUnits();
}
